<div>
        ...
        @foreach ([1 => 'Verify Email', 2 => 'Personal Info', 3 => 'Password'] as $num => $label)
            <div class="flex items-center gap-2">
                <div class="flex flex-col items-center gap-1">
                    ...
                    <span class="text-[10px] font-medium text-text-muted whitespace-nowrap">{{ $label }}</span>
                </div>
                @if ($num < 3)
                    <div class="w-8 h-0.5 {{ $currentStep > $num ? 'bg-primary-dark' : 'bg-border-subtle' }} mb-4"></div>
                @endif
            </div>
        @endforeach
    </div>
        ...
        <div class="max-w-md mx-auto space-y-5">
            ...
            @if (! $codeSent)
                <button wire:click="sendCode" wire:loading.attr="disabled" wire:target="sendCode"
                    class="w-full py-3 px-4 bg-primary hover:bg-primary-dark text-surface text-sm font-bold rounded-xl shadow-md transition-colors disabled:opacity-60">
            ...
            @else
                <div>
                    ...
                    <p class="text-text-muted text-xs mt-1">We sent a 6-digit code to {{ $email }}.</p>
                </div>
                ...
                <div class="text-center" x-data="{ seconds: {{ $resendCooldown }} }"
                    ...
                    <button wire:click="resendCode" wire:loading.attr="disabled" wire:target="resendCode"
                        ...
                        class="text-xs font-bold text-primary hover:text-primary-dark transition-colors disabled:text-text-muted disabled:cursor-not-allowed">
    ...
    @if ($currentStep === 2)
        <div class="max-w-2xl mx-auto grid grid-cols-1 md:grid-cols-12 gap-5">
            ...
            <div class="md:col-span-12 flex gap-3 mt-2">
                <button wire:click="backToStep(1)" type="button"
                    class="flex-1 py-3 px-4 bg-surface-subtle hover:bg-border-subtle text-text-main text-sm font-bold rounded-xl transition-colors">
                    Back
                </button>
                <button wire:click="goToPasswordStep" wire:loading.attr="disabled" wire:target="goToPasswordStep"
                    class="flex-[2] py-3 px-4 bg-primary hover:bg-primary-dark text-surface text-sm font-bold rounded-xl shadow-md transition-colors disabled:opacity-60">
                    Continue
                </button>
            </div>
        </div>
