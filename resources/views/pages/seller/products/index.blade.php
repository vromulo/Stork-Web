...
<body class="m-0 p-0 h-screen w-screen font-sans antialiased text-text-main overflow-hidden bg-gradient-to-b from-surface via-surface to-brand-light/30">
    ...
    <!-- Toast Popup for Success Messages (2 Second Timer) -->
    @if(session('success'))
                        ...
                        <table class="w-full text-left border-collapse relative">
                            ...
                            <tbody>
                                ...
                                <tr x-data="{ 
                                    ...
                                    <td class="p-4">
                                        <span class="px-3 py-1 rounded-full text-xs font-bold {{ $product->stock_quantity > 0 ? 'bg-green-100 text-green-700 border border-green-200' : 'bg-red-100 text-red-700 border border-red-200' }}">
                                    ...
                                    <!-- 1. E-COMMERCE AESTHETIC VIEW MODAL -->
                                                                    ...
                                                                    <div class="flex flex-wrap gap-2">
                                                                        @foreach($product->variants['items'] as $vIndex => $item)
                                                                            <button @click.stop="
                                                                                    ...
                                                                                    class="px-4 py-2 text-sm font-bold border-2 rounded-xl transition-all cursor-pointer">
                                                                                {{ $item['name'] ?? 'Unnamed' }}
                                                                            </button>
                                                                        @endforeach
                                                                    </div>
                                            ...
                                            <div @click.away="editModal = false" class="bg-white rounded-3xl w-full max-w-lg shadow-2xl overflow-hidden flex flex-col cursor-default">
                                                ...
                                                <form action="{{ route('seller.products.update', $product->id) }}" method="POST" @click.stop class="p-6 space-y-5 overflow-y-auto max-h-[75vh] custom-scrollbar">
                                                    ...
                                                    <div class="grid grid-cols-2 gap-4">
                                                        <div>
                                                            <label class="block text-sm font-bold text-text-muted mb-1">Stock Quantity</label>
                                                            <input type="number" name="stock_quantity" value="{{ $product->stock_quantity }}" required class="w-full p-3 rounded-xl border border-border-subtle focus:border-primary outline-none">
                                                        </div>
                                                        <div>
